"""This is an example of a bad test file.

:et's start with the fact that we prefer VCR tests, but this file is testing edge cases, all of which should fail validation before even making an API call.
My feedback to this test is the following:

1. imports go at the top, not inline
2. because we're testing different models we need the classic  `try_import` guards and skipmarks on each test to be skipped if their import fails
3. I dislike **Creating** the modelparameteres and the calling the underlying function directly, it's a bit of a cheat becasue theere's a lot of implicit logic in the agent initialization that I don't wanna just forego, because we might miss hidden behavior. So please get rid of that and add the tool etc as they would be in the public API
"""
...
def test_auto_without_text_output():
    params = ModelRequestParameters(
        function_tools=[make_tool("x")], allow_text_output=False
    )
...
def test_none_with_text_output_allowed():
    params = ModelRequestParameters(
        function_tools=[make_tool("x")], allow_text_output=True
    )
...
def test_list_all_invalid_raises():
    ...
    params = ModelRequestParameters(
        function_tools=[make_tool("a"), make_tool("b")], allow_text_output=True
    )
...
def test_list_subset_returns_tuple():
    params = ModelRequestParameters(
        function_tools=[make_tool("a"), make_tool("b"), make_tool("c")],
        allow_text_output=True,
    )
...
def test_tool_or_output_empty_function_tools_with_output_tools_direct_output():
    ...
    result = resolve_tool_choice(
        {"tool_choice": ToolOrOutput(function_tools=[])}, params
    )
    assert result == "auto"
...
def test_tool_or_output_all_invalid_function_tools_silently_passed():
    ...
    result = resolve_tool_choice(
        {"tool_choice": ToolOrOutput(function_tools=["x", "y"])}, params
    )
...
def test_tool_or_output_partial_invalid_function_tools_raises():
    ...
    with pytest.raises(UserError, match="Invalid tool names"):
        resolve_tool_choice(
            {"tool_choice": ToolOrOutput(function_tools=["a", "invalid"])}, params
        )
...
def test_tool_or_output_exact_match_returns_required():
    ...
    result = resolve_tool_choice(
        {"tool_choice": ToolOrOutput(function_tools=["a"])}, params
    )
    assert result == "required"
...
async def test_anthropic_thinking_with_forced_tool_choice_raises(
    tool_choice: ToolChoice, allow_model_requests: None
):
    ...
    m = AnthropicModel(
        "claude-sonnet-4-5", provider=AnthropicProvider(api_key="test-key")
    )
    ...
    with pytest.raises(
        UserError, match="Anthropic does not support .* with thinking mode"
    ):
...
async def test_bedrock_unsupported_profile_with_forced_tool_choice_raises(
    tool_choice: ToolChoice, allow_model_requests: None
):
    ...
    m = BedrockConverseModel(
        "us.amazon.nova-lite-v1:0", provider=provider, profile=profile
    )
...
async def test_openai_unsupported_profile_with_forced_tool_choice_raises(
    tool_choice: ToolChoice, allow_model_requests: None
):
    """Models without tool_choice support raise UserError when forcing tool use."""
    ...
    m = OpenAIChatModel("gpt-5-mini", provider=provider, profile=profile)
...
def test_google_auto_tuple_filters_tool_defs():
    """When resolve_tool_choice returns ('auto', [...]), Google filters tool_defs."""
    from pydantic_ai.models.google import GoogleModel
    from pydantic_ai.providers.google import GoogleProvider

    mock_client = MagicMock()
    provider = GoogleProvider(client=mock_client)
    m = GoogleModel("gemini-3-flash-preview", provider=provider)
    ...
    assert len(tools) == 1
    assert tools[0]["function_declarations"][0]["name"] == "final_result"
    assert tool_config is not None
    assert tool_config["function_calling_config"]["mode"].name == "AUTO"


# =============================================================================
# xAI tool_choice tests
# =============================================================================


def test_xai_fallback_single_tool_without_required_support():
    ...
    m = XaiModel("grok-4-fast", provider=provider, profile=profile)

    params = ModelRequestParameters(
        function_tools=[make_tool("tool_a"), make_tool("tool_b")],
        allow_text_output=True,
    )
...
def test_xai_fallback_multiple_tools_without_required_support():
    ...
    params = ModelRequestParameters(
        function_tools=[make_tool("tool_a"), make_tool("tool_b"), make_tool("tool_c")],
        allow_text_output=True,
    )

    tool_defs, tool_choice = m._get_tool_choice(
        {"tool_choice": ["tool_a", "tool_c"]}, params
    )  # pyright: ignore[reportPrivateUsage]
    assert tool_choice == "auto"
    assert set(tool_defs.keys()) == {"tool_a", "tool_c"}


def test_xai_required_with_no_text_output_and_supported():
    ...
    params = ModelRequestParameters(
        function_tools=[make_tool("tool_a")],
        allow_text_output=False,
    )
